...
En revanche, l'utilisation de RMSProp pour le générateur et d'Adam pour le discriminateur, ou Adam pour les deux, ne fonctionne pas. Cela corrobore les résultats présentés par d'autres \cite{auriau_apprentissage_2017}[ref nécessaire : autre ref].
L'utilisation d'Adam pour le générateur et une gradient standard pour le discriminateur [doit être testé par Kv]
L'interprétation est la suivante : il est nécessaire d'apprendre davantage au générateur qu'au discriminateur. Autrement, le générateur devient perdu.
...
Certaines fuites de threads dues au multithreading manuel avec les smartpointers ont été présentes par moment, mais uniquement sur une ou deux des quatre machines.
